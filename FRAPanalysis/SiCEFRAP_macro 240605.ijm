...
////parameters selection
	...
	Dialog.addNumber("FRAP zone lenght",40, 0,3, "pixels");
	Dialog.addNumber("Membrane thickness",10, 0,3, "pixels");
	Dialog.addChoice("Orientation of bleaching", newArray("Lateral Membrane", "Apico basal Membrane","cortical"));
	Dialog.addNumber(" Photobleaching at frame nb",5, 0,3, " ");
	Dialog.addNumber(" Find max proheminence",100, 0,3, " ");
...
//background removal	
		...
		rename("Z-SUM projection");
		Dialog.create(" ");
		Dialog.addMessage("Any visible XY Drift in the Z-SUM projection ?");
		Dialog.addCheckbox("Run Fast4DReg", false);
		Dialog.show();
...
// Root segmentation using wavelet (for Lateral or Apicobasal FRAP)
	...
	if (ori!="cortical"){	
			...
			if (select==-1)
						{
				setTool("line");	
				waitForUser("Draw a line in cell main axis");
				run("Measure");
						}
