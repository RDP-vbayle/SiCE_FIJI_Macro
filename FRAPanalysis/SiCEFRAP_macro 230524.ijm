...
////parameters selection
	...
	Dialog.addNumber("FRAP zone lenght",50, 0,3, "pixels");
	...
	Dialog.addNumber(" prebleach image number",10, 0,3, " Frame");
	Dialog.addNumber(" Find max proheminence",1000, 0,3, " ");
	Dialog.addNumber(" ROIs nb",5, 0,3, " ");
	Dialog.show();
...
// Noise substraction
	...
	setBatchMode(true);
		...
		run("Clear Results");
		roiManager("reset");	
		run("Set Measurements...", "area fit display redirect=None decimal=5");
...
// bleached ROIs identification
		...
		run("Convert to Mask");
		selectWindow("Result of 1");
		run("Analyze Particles...", "size="+(3*zone)+"-"+(50*zone)+"  exclude add");
