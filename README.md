...
1. Pre-Processing: In our case some frames shows "Tilting", meaning the position on the spiro is not facing the camera and there is a little shift. Here is a macro to correct this: [ShiftCorrection.ijm](RootGravi). Import the time lapse on FIJI File>Import. Launch the macro. Save the different rows of seedlings separately.
