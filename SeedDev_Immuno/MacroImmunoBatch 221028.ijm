...
///Creation du tableau de résultat
	...
	print(f,"\\Headings: Image name \t Background \t Embryo stage \t Seed Number \t Cell Layer\t Cell number \t Mean Mb fluorescence ");
...
/// Extraction d'info à partir du titre	
	background=substring(title,0,lastIndexOf(title,"dpa")-1);
	stage=substring(title,lastIndexOf(title,"dpa")-1,lengthOf(title));
	seedNb=1;		
	...
	setBatchMode(false);
		...
		for (i = 0; i < Lnb; i++) {
			...
			p++;
						...
						for (m = 0; m < totRois; m++) {
							print(f, title+"\t"+ background +"\t"+stage+"\t"+seedNb+"\t"+Celllayer[m] +"\t"+m+1+"\t"+Cellfluo[m]);			
						}
		roiManager("reset");
		run("To ROI Manager");
		run("Green");
