...
Dialog.create("SiCE Root hair macro");
	Dialog.addNumber("Min Root hair size",60, 1,5, "pixels^2");
	Dialog.addNumber("Rolling background size",200, 1,5, " ");
	Dialog.addNumber("RHseg LowSigma DoG",3, 2,2, " ");
	Dialog.addNumber("RHseg HighSigma DoG",5, 2,2, " ");
	Dialog.addNumber("Mean Threshold",14000, 0,6, " ");
...
// function description
	...
	/// DoG	
		...
		rename("2");
		selectImage("1");
		run("Gaussian Blur...", "sigma="+S3);
		selectImage("2");
		run("Gaussian Blur...", "sigma="+S4);
		...
		roiManager("deselect");
		run("Fill Holes");
		makeOval(tipX-200, tipY-200, 400, 400);	
		run("Clear", "slice");
		run("Select None");
