///////Macro for Dots quantification in WT vs plasmolysed cells. INPUT: Ilastik segmented
...
///Creation du tableau de résultat
	...
	print(f,"\\Headings: Image name \t ROI number \t Cell Type \t ROI Area \t Nb Dots \t Density \t Average CS size");
	title2 = "[Contact size:]"; 
	g=title2; 
...
/// Cellfile type & ROI selection
	...
	close();		
		...
		Dialog.addChoice(" ", newArray("Cotyledon","Hypocotyle"));
		Dialog.show();				
		CellType = Dialog.getChoice();		
		selectWindow("CS");
		roiManager("select", 0);	
		run("Analyze Particles...", "size="+min+"-Infinity display clear include summarize");	
		...
		print(f, title+"\t"+ i+"\t"+CellType+"\t"+CellArea+"\t"+nbDots+"\t"+nbDots/CellArea +"\t"+AVsize);	
