...
Dialog.create("SiCE Root hair macro");
	Dialog.addNumber("Min Root hair size",5, 1,5, "pixels^2");
	Dialog.addNumber("Rolling background size",200, 1,5, " ");
	Dialog.addNumber("Variance filter size (Main root seg)",30, 1,5, " ");
	Dialog.addNumber("Top hat filter size (RH seg)",5, 1,5, " ");
	Dialog.addNumber("Mean Threshold",5000, 0,6, " ");
...
/// Result table
	...
	print(f,"\\Headings: Image name \t Root Hair nb \t Distance from Root tip \t Length \t Out of Focus \t Branched \t X centroid \t Y centroid \t Mean \t SD");
...
// function description
	...
	/// Root segmentation
		...
		RootSegment(title, rootCheck,va);
	...
	/// TOPhat	
		...
		roiManager("deselect");
		//run("Fill Holes");
		makeOval(tipX-100, tipY-100, 200, 200);	
		run("Clear", "slice");
		run("Select None");
...
/// Arrays creation for measurements		
		...
		SD = newArray(count-2);
		Mean = newArray(count-2);
		
	selectWindow("Temp");
	RHfiltering(OOF,Dist,Length,MeanT,Xcent, Ycent,Mean, SD);
	...
	print(f,title+"\t"+(n+1)+"\t"+Dist[n]+"\t"+Length[n]+"\t"+OOF[n]+"\t"+Branch[n]+"\t"+Xcent[n]+"\t"+Ycent[n]+"\t"+Mean[n]+"\t"+SD[n]);
	run("Clear Results");
	...
	print(f,title+"\t No Root hairs\t ND \t ND \t ND \t ND \t ND \t ND \t ND \t ND"); 
...
function RHfiltering(OOF,Dist,Length,MeanT,Xcent,Ycent,Mean, SD) {
	run("Clear Results");
	count = roiManager("count");	
	run("Clear Results");
	for (m = 0; m < count-2; m++) {
	...
	Mean[m]= getResult("Mean", 0);
	SD[m]= getResult("StdDev", 0);
	ratio=Mean[m]/SD[m];
	if (ratio<2 || MeanRH<MeanT) {
		OOF[m]="Out of Focus";
	}
	else {
		OOF[m]="In Focus";
...
function RootSegment(title,rootCheck,va){
	getDimensions(width, height, channels, slices, frames);
//	selectImage(title);
	run("Duplicate..."," ");
	rename("tempBis");
